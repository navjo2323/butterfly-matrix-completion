...
def sort_inds_and_T(tuples_list, subset_indices, T):
    """
    Sorts a list of tuples based on the values of a subset of indices, returns the sorted list 
    and counts of each unique subset, and reorders the 1D array T according to the sorted tuples.
    
    Parameters:
    tuples_list (list of tuples): The list of tuples to be sorted.
    subset_indices (list of int): A list of indices based on which to sort.
    T (numpy array): A 1D numpy array that needs to be reordered according to the sorted tuples.
    
    Returns:
    tuple: A tuple containing:
        - list of tuples: The sorted list of tuples.
        - list of tuples: Each unique subset (according to subset_indices) with its count.
        - numpy array: The reordered 1D numpy array.
    """
    # Sort tuples_list based on subset_indices, but return the sorted indices
    sorted_indices = sorted(range(len(tuples_list)), key=lambda i: tuple(tuples_list[i][j] for j in subset_indices))
    sorted_tuples = [tuples_list[i] for i in sorted_indices]
    
    # Reorder the 1D numpy array T based on the sorted indices
    reordered_T = T[sorted_indices]
    
    # Dictionary to keep track of counts of each unique subset
    subset_count = defaultdict(int)
    
    # Count occurrences of each subset
    for t in sorted_tuples:
        subset_key = tuple(t[i] for i in subset_indices)
        subset_count[subset_key] += 1
    
    # Convert the count dictionary into a sorted list of tuples (subset, count)
    subset_counts_list = sorted(subset_count.items())
    
    return sorted_tuples, subset_counts_list, reordered_T
...
def get_subset(L,level,r_c):
    # These are always present
    if r_c ==0:
        subset = [i for i in range(L//2 + 1)]
        more_indices1 = level - L//2 
        if level !=L:
            more_indices1 += 1
        more_indices2 = L - level 
        subset += [i for i in range(L//2 +1, L//2 + more_indices1,1)]
        subset += [i for i in range(L, L + more_indices2,1)]

        if level == L:
            subset += [2*L]

    else:
        subset = [i for i in range(L, L + L//2 +1,1)]
        more_indices1 = level - L//2
        if level !=L:
            more_indices1 += 1
        more_indices2 = L - level
        subset += [i for i in range( L + L//2 +1,L +  L//2 + more_indices1,1)]
        subset = [i for i in range(more_indices2)] + subset
...
def new_ALS_solve(T,inds,g_lst,h_lst,level,L,lc,r_c,regu):
    ...
    if lst_ind ==0:
        if r_c == 0:
            row_shape = g_lst[lst_ind].shape[-1]
        else:
            row_shape = h_lst[lst_ind].shape[-1]
    else:
        if r_c ==0:
            row_shape = g_lst[lst_ind].shape[-1]*g_lst[lst_ind].shape[-2]
        else:
            row_shape = h_lst[lst_ind].shape[-1]*h_lst[lst_ind].shape[-2]

    s = time.time()

    subset_for_solve = get_subset(L,level,r_c)

    sorted_tuples, subset_counts, T_new = sort_inds_and_T(inds, subset_for_solve,T)

    e = time.time()

    #For now creating a new copy of indices and T

    print('Time in sorting',e-s)



    # Going through each row that would be solved
    ...
    for i in range(len(subset_counts)):
        ...
        # A left hand side 
        # A right hand side
        LHS = np.zeros((row_shape,row_shape))
        RHS = np.zeros(row_shape)

        H = np.zeros((subset_counts[i][1],row_shape))

        # Further can be optimized based on sorted indices
        # For now lets keep it this way
        for k in range(subset_counts[i][1]):
            H[k,:] = multiply_mats(g_lst,h_lst,level,L,lc,r_c,sorted_tuples[cumulate + k]).reshape(-1)

        #matrix vector product v^TH to get right hand sides
        #RHS = np.einsum('i,ij->j',T_new[cumulate: cumulate+ subset_counts[i][1] ],H,optimize=True)
        RHS = T_new[cumulate: cumulate+ subset_counts[i][1] ].T @ H

        cumulate += subset_counts[i][1]

        # Matrix-Matrix H^TH product to get symmetric LHS
        #LHS = np.einsum('ij,ik->jk',H,H,optimize=True)

        LHS = H.T @ H

        if lst_ind == 0:
            if r_c ==0:
                g_lst[lst_ind][subset_counts[i][0] + (slice(None),)] = la.solve(LHS + regu*np.eye(LHS.shape[0]), RHS)
            else:
                h_lst[lst_ind][subset_counts[i][0] + (slice(None),)] = la.solve(LHS + regu*np.eye(LHS.shape[0]), RHS)
        else:
            if r_c == 0:
                g_lst[lst_ind][subset_counts[i][0] + (slice(None),slice(None))] = la.solve(LHS + regu*np.eye(LHS.shape[0]), RHS).reshape((g_lst[lst_ind].shape[-2],g_lst[lst_ind].shape[-1]))
            else:
                h_lst[lst_ind][subset_counts[i][0] + (slice(None),slice(None))] = la.solve(LHS + regu*np.eye(LHS.shape[0]), RHS).reshape((h_lst[lst_ind].shape[-2],h_lst[lst_ind].shape[-1]))


    return g_lst,h_lst


#Define process_chunk at the top level (outside of the new_ALS_solve function)
def process_chunk(chunk, T_new, g_lst, h_lst, level, L, lc, r_c, regu, row_shape):
    subset_count, sorted_tuples, cumulate = chunk

    # Perform the ALS solve logic here
    LHS = np.zeros((row_shape, row_shape))
    RHS = np.zeros(row_shape)
    H = np.zeros((subset_count[1], row_shape))

    for k in range(subset_count[1]):
        H[k, :] = multiply_mats(g_lst, h_lst, level, L, lc, r_c, sorted_tuples[cumulate + k]).reshape(-1)

    RHS = np.einsum('i,ij->j', T_new[cumulate: cumulate + subset_count[1]], H, optimize=True)
    LHS = np.einsum('ij,ik->jk', H, H, optimize=True)

    lst_ind = L - level
    if lst_ind == 0:
        if r_c == 0:
            g_lst[lst_ind][subset_count[0] + (slice(None),)] = la.solve(LHS + regu * np.eye(LHS.shape[0]), RHS)
        else:
            h_lst[lst_ind][subset_count[0] + (slice(None),)] = la.solve(LHS + regu * np.eye(LHS.shape[0]), RHS)
    else:
        if r_c == 0:
            g_lst[lst_ind][subset_count[0] + (slice(None), slice(None))] = la.solve(LHS + regu * np.eye(LHS.shape[0]), RHS).reshape(g_lst[lst_ind].shape[-2], g_lst[lst_ind].shape[-1])
        else:
            h_lst[lst_ind][subset_count[0] + (slice(None), slice(None))] = la.solve(LHS + regu * np.eye(LHS.shape[0]), RHS).reshape(h_lst[lst_ind].shape[-2], h_lst[lst_ind].shape[-1])

    return total_c

# # Main ALS function using separate memory copies
# def new_ALS_solve(T, inds, g_lst, h_lst, level, L, lc, r_c, regu, num_processes):
#     assert (level <= L and level >= lc)

#     lst_ind = L - level
#     row_shape = (g_lst[lst_ind].shape[-1] if r_c == 0 else h_lst[lst_ind].shape[-1])

#     # Prepare subsets for processing
#     subset_for_solve = get_subset(L, level, r_c)
#     sorted_tuples, subset_counts, T_new = sort_inds_and_T(inds, subset_for_solve, T)
...
#     for i in range(len(subset_counts)):
#         args_list.append((
#             subset_counts[i], sorted_tuples, cumulate, total_c
#         ))
#         cumulate += subset_counts[i][1]
#         total_c += subset_counts[i][1]

#     # Run multiprocessing pool
#     with ProcessPoolExecutor(max_workers=num_processes) as executor:
#         chunk_results = executor.map(process_chunk, args_list, 
#                                      [T_new] * len(args_list), [g_lst] * len(args_list), [h_lst] * len(args_list),
#                                      [level] * len(args_list), [L] * len(args_list), 
#                                      [lc] * len(args_list), [r_c] * len(args_list), 
#                                      [regu] * len(args_list), [row_shape] * len(args_list))
...
# Vectorized above function, is a faster than both
def create_inds(I, J, nnz, rng):
    unique_tuples = set()
    while len(unique_tuples) < nnz:

...
from tensor_train_decomp import tensor_train_decomposition_low, tensor_train_decomposition, convert_matrix_to_QTT_indices, tensor_train_completion, ADAM_tensor_train
...
s = time.time()
qtt_inds = convert_matrix_to_QTT_indices(L,c,indices)
...
num_iters = 20
s = time.time()
tensor_lst = ADAM_tensor_train(T_sparse, qtt_inds, T_sparse_test, qtt_inds_test, L, factors, regu = 1e-9, max_iter=num_iters, tol=tol)
e = time.time()
